...
from skan import Skeleton, draw, skeleton_to_csgraph, summarize
from skimage import color, data, feature, filters, io
from skimage.measure import label, regionprops
from skimage.morphology import (binary_closing, binary_dilation, dilation,
...
def endPoints(skel):
    ...
    endpoint9 = np.array([[0, 0, 0], [0, 1, 1], [1, 0, 1]])
...
#### Function that finds the endpoints where the prunning will start
def endPoints2(skel):
    endpoint1 = np.array([[1, 0, 0], [1, 1, 0], [2, 0, 0]])

    endpoint2 = np.array([[0, 0, 1], [0, 1, 1], [0, 0, 2]])

    endpoint3 = np.array([[0, 1, 0], [1, 1, 0], [2, 0, 0]])

    endpoint4 = np.array([[0, 1, 0], [0, 1, 1], [0, 0, 2]])
    endpoint5 = np.array([[0, 0, 0], [1, 1, 0], [0, 1, 0]])
...
def Threshold_Demo(val):
    ...
    _, dst = cv2.threshold(img, threshold_value, max_binary_value, threshold_type)
...
def Adaptive_Threshold_Demo(val):
    ...
    thresh = cv2.adaptiveThreshold(
        gray, 255, method, cv2.THRESH_BINARY, block_size, c_value
    )
...
with open(csvPath + "arteryArtTort.csv", mode="w+") as csv_file:
    ...
    for filename in os.listdir(arteryPath):
        ...
        if filename.endswith(".tif"):
            ...
            # img = cv2.GaussianBlur(gray, (7, 7), cv2.BORDER_DEFAULT)
            ...
            cv2.createTrackbar(
                trackbar_type, window_name, 0, max_type, Adaptive_Threshold_Demo
            )
            cv2.createTrackbar(
                trackbar_block_size, window_name, 3, max_block, Adaptive_Threshold_Demo
            )
            # Create Trackbar to choose Threshold value
            cv2.createTrackbar(
                trackbar_c_value, window_name, 2, max_c, Adaptive_Threshold_Demo
            )
            ...
            _, contours0, hierarchy = cv2.findContours(
                contour_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
            )
