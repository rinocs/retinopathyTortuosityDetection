    ...
    for filename in os.listdir(skeletonPath):
        ...
        if filename.endswith(".png"):
            ...
            for region in regionprops(labels):
                ...
                mean_dm, num_inflection_pts, curvature, DM = (
                    tortuosity.mean_distance_measure(x_coords, y_coords)
                )
            ...
            # compute mean distance measure (ratio of actual length to chord length) between inflection points
            mat_mean_dm, mat_num_inflection_pts, mat_curvature, mat_DM = (
                tortuosity.mean_distance_measure(mat_x_coords[0], mat_y_coords[0])
            )

            # compute number of critical points
            mat_num_cpts = tortuosity.num_critical_points(
                mat_x_coords[0], mat_y_coords[0]
            )

            # compute VTI index
            mat_VTI = tortuosity.vessel_tort_index(
                mat_curve_length, mat_sd, mat_num_cpts, mat_mean_dm, mat_chord_length
            )
            writer.writerow(
                {
                    "image": filename,
