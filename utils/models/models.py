...
def createModelOne(inputShape=(100, 100, 1), NUM_CLASSES=1, INIT_LR=1e-3, EPOCHS=100):
    model = Sequential()
...
def train_model(image_size, batch_size=50, nb_epoch=20):
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(
        train_data, train_target, test_size=cv_size, random_state=56741
    )
    ...
    compare = pd.DataFrame(
        data={
            "original": y_valid.reshape((cv_size,)),
