...
for i in range(0, len(seg_art[0]) - 1):
    ...
    linear_reg_tort = tortuosity.linear_regression_tortuosity(
        x_coords[0], y_coords[0], 50
    )
    squared_tort = tortuosity.squared_curvature_tortuosity(x_coords[0], y_coords[0])
    distance_inflection_tort = tortuosity.distance_inflection_count_tortuosity(
        x_coords[0], y_coords[0]
    )
    ...
    y = {
        str(name): {
            ...
            "rank": int(np.squeeze(rank)) if isinstance(rank, np.ndarray) else int(rank),
        }
    }
